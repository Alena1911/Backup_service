...
namespace Backup_service
{
    internal class Ftp_Client
    {
        ...
        public FileStruct[] ListDirectory(string path)
        {
            ...
            //кодировка автоматически не определяется, указываем 1251, если в будущем будут проблемы с кодировкой,
            //поменять на System.Text.Encoding.ASCI
            StreamReader sr = new StreamReader(ftpResponse.GetResponseStream(), System.Text.Encoding.GetEncoding(1251));
    ...
    public class DirectoryListParser
    {
        ...
        public FileListStyle GuessFileListStyle(string[] recordList)
        {
            foreach (string s in recordList)
            {
                ...
                if (s.Length > 10
                    && Regex.IsMatch(s.Substring(0, 10), "(-|d)((-|r)(-|w)(-|x|s)){3}"))
                {
